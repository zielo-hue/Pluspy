...
namespace System.Security.Cryptography
{
    ...
    public sealed class RijndaelManagedTransformCore : ICryptoTransform
    {
        ...
        internal RijndaelManagedTransformCore(byte[] rgbKey,
                                           CipherMode mode,
                                           byte[] rgbIV,
                                           int blockSize,
                                           int feedbackSize,
                        ...
                        fixed (int* TF = s_TF)
                        {
                            ...
                            for (int blockNum = 0; blockNum < iNumBlocks; ++blockNum)
                            {
                                ...
                                if (m_cipherMode == CipherMode.CFB) {
                                    ...
                                    Span<byte> workSpan = new Span<byte>(work, m_blockSizeBytes);
                                    m_shiftRegister.AsSpan().Slice(0, m_blockSizeBytes).CopyTo(workSpan);
                                } else {
#endif // FEATURE_CRYPTO
                                if (blockNum != iNumBlocks - 1 || padSize == 0)
                                {
                                    Contract.Assert(m_blockSizeBytes <= m_Nb * sizeof(int), "m_blockSizeBytes <= m_Nb * sizeof(int)");
                                        //Buffer.MemoryCopy((byte*)work, 0, inputBuffer, workBaseIndex, m_blockSizeBytes);
                                        Span<byte> workSpan = new Span<byte>(work, m_blockSizeBytes);
                                        m_shiftRegister.AsSpan().Slice(workBaseIndex, m_blockSizeBytes).CopyTo(workSpan);
                                    }
                                else
                                {
                                    int padIndex = 0;
                                ...
                                fixed (int* pLastBlockBuffer = m_lastBlockBuffer)
                                {
                                    ...
                                    Buffer.MemoryCopy((byte*)pLastBlockBuffer, (byte*)temp, m_blockSizeBytes, m_blockSizeBytes);
                                }
#if FEATURE_CRYPTO
                                    }
                                }
